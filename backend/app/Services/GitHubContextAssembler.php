    ...
    public function assemble(GitHubReference $ref, array $raw): array
    {
        $context = [
            ...
            'repository' => [
                ...
                'readme' => $raw['readmeContent'] !== null ? mb_substr($raw['readmeContent'], 0, 50000) : null,
                'file_tree' => array_slice(array_column($raw['tree'], 'path'), 0, 1000),
            ],
        ];
        ...
        if ($ref->type === 'pull_request' && $raw['pr'] !== null) {
            ...
            $context['changed_files'] = array_map(
                fn (array $f) => ['name' => $f['filename'], 'status' => $f['status'], 'diff' => mb_substr($f['patch'] ?? '', 0, 8000)],
                array_slice($raw['prFiles'], 0, 100),
            );
            $context['comments'] = array_map(
                fn (array $c) => ['author' => $c['user']['login'] ?? null, 'body' => mb_substr($c['body'] ?? '', 0, 5000)],
                array_slice($raw['prComments'], 0, 50),
            );
