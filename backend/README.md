...
**Turso (libSQL):** set `DB_CONNECTION=libsql`, `TURSO_DATABASE_URL` (e.g. `libsql://ai-flow-jellydn.aws-ap-northeast-1.turso.io`), and `TURSO_AUTH_TOKEN` from `turso db tokens create <db-name>`. The app uses `turso/libsql-laravel` (remote-only on Cloud: leave `TURSO_LOCAL_DATABASE` unset). PHP **ext-ffi** must be enabled on the Cloud runtime; if deploy boots but HTTP 500s with an FFI error, use Laravel Cloud [build configuration](https://cloud.laravel.com/docs/private-cloud/build-configuration) or managed Postgres/MySQL instead.
